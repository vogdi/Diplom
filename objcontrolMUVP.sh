...
do
     ...
     do
        ...
        sshpass -p 'transas' scp -r -v -o StrictHostKeyChecking=no root@muvp-capture:"$testPath""$Date""$dayData""$testPath1""$dayHoreStart""$testPath1""$dayMinutStartC""$testPath2" "$localPath" #2>/dev/null
        else
        break
        fi
...
function checkFolder {
    read -r -p "Введите дату(день например 05 или 15): " dayData
    checkData
    localPath="$mountPath/MUVP/$Date$dayData"
...
sshpass -p 'transas' scp -r -v -o StrictHostKeyChecking=no root@muvp-node03:/var/lib/deployment/orion/icd_logs/RecorderIcd_*__$logDate* $logPath
umount $mountPath
